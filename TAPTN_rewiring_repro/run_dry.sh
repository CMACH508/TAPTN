#!/bin/bash
# Dry-run of the rewiring tables (no LLM calls).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")" && pwd)"
if [[ -z "${TAPTN_ASSETS:-}" ]]; then
  if [[ -d "$ROOT/../TAPTN_rewiring_repro_assets" ]]; then
    export TAPTN_ASSETS="$ROOT/../TAPTN_rewiring_repro_assets"
  elif [[ -d "$ROOT/assets" ]]; then
    export TAPTN_ASSETS="$ROOT/assets"
  else
    echo "Unpack the asset bundle and set TAPTN_ASSETS, e.g.:" >&2
    echo "  export TAPTN_ASSETS=/path/to/TAPTN_rewiring_repro_assets" >&2
    exit 1
  fi
fi
