...
# PyTorch 2.6 将 torch.load 默认 weights_only=True，
# 但 torch_geometric 内部仍使用旧 API，导致含 PyG Data 对象的预处理文件加载失败。
# 解决方案：在 PyTorch 2.4+ 上将常用 PyG 类注册为安全全局。
def _register_pyg_safe_globals():
    if not hasattr(torch.serialization, 'add_safe_globals'):
        return  # PyTorch < 2.4，无需处理
    classes = []
    try:
        from torch_geometric.data.data import Data, GlobalStorage
        classes += [Data, GlobalStorage]
    except ImportError:
        pass
    try:
        from torch_geometric.data.storage import GlobalStorage as GS
        if GS not in classes:
            classes.append(GS)
    except ImportError:
        pass
    try:
        from torch_geometric.data.data import DataEdgeAttr, DataTensorAttr
        classes += [DataEdgeAttr, DataTensorAttr]
    except ImportError:
        pass
    if classes:
...
def get_raw_text_cora(use_text=False, seed=0):
    ...
    for pid in data_citeid:
        fn = pid_filename[pid].replace(':', '_')
        fn = all_files.get(fn.lower(), None)
