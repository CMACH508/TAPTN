...
import csv
from torch_geometric.nn import GATConv,DirGNNConv,GCNConv
...
class GAT(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, dropout, use_pred):
        ...
        self.convs.append(DirGNNConv(GCNConv(in_channels, hidden_channels)))
        self.bns = torch.nn.ModuleList()
        self.bns.append(torch.nn.BatchNorm1d(hidden_channels))
        for _ in range(num_layers - 2):
            self.convs.append(GATConv(hidden_channels, hidden_channels))
            self.bns.append(torch.nn.BatchNorm1d(hidden_channels))
        self.convs.append(DirGNNConv(GCNConv(hidden_channels, out_channels)))
        if self.use_pred:
